...
A second TXT on `@`, `google-site-verification=0Slu56bQ0tPf47dXRTxgK0F7QFHY1PmTORyiJqgFkvg`, verifies
the domain in Google Search Console (property `sc-domain:swissnaturalizationtest.ch`, sitemap submitted
16.09.2026). The file `efd6a79ef530391ba15b61562b18bd7e.txt` at the site root is the IndexNow key.
Neither may be removed. Search metadata is generated by `_tools/seo.py`.

**The TXT records stay forever.** It proves to GitHub that the account owns the domain;
